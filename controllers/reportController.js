  ...
  try {
    ...
    const report = Object.keys(grouped).map(empId => {
      ...
      workingDays.forEach(d => {
        ...
        attendanceTotal += rec
          ? getAttendanceScore(rec.status)
          : 0;

        punctualityTotal += rec
          ? getPunctualityScore(rec.check_in_time)
          : 0;

        daysCount++;
      });

      const attendanceRate = daysCount
        ? Math.round(attendanceTotal / daysCount)
        : 0;

      const punctualityRate = daysCount
        ? Math.round(punctualityTotal / daysCount)
        : 0;

      const performance = Math.round(
        (attendanceRate + punctualityRate) / 2
      );
